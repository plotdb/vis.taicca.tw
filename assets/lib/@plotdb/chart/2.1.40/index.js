(function(){
  var svgns, _np, _nparse, parent, aniloop, visibilityObserver, resizeObserver, withDefault, defaultConfig, chart;
  svgns = "http://www.w3.org/2000/svg";
  _np = function(lc){
    var format, parts, numerals, index;
    format = new Intl.NumberFormat(lc);
    ...
    index = new Map(numerals.map(function(d, i){
      return [d, i];
    }));
    this._group = new RegExp("[" + parts.find(function(d){
      return deepEq$(d.type, 'group', '===');
    }).value + "]", 'g');
    this._decimal = new RegExp("[" + parts.find(function(d){
      return deepEq$(d.type, 'decimal', '===');
    }).value + "]");
  ...
  withDefault = function(cfg, defcfg, meta){
    var _;
    cfg == null && (cfg = {});
    defcfg == null && (defcfg = {});
    meta == null && (meta = {});
    defcfg = JSON.parse(JSON.stringify(defcfg));
    _ = function(cfg, defcfg, meta){
      var k, v;
      for (k in defcfg) {
        v = defcfg[k];
        if (!(cfg[k] != null)) {
          cfg[k] = v;
        } else if (typeof cfg[k] === 'object' && typeof v === 'object' && !(meta[k] && meta[k].type)) {
          withDefault(cfg[k], defcfg[k], meta[k]);
        }
      }
      return cfg;
    };
    return _(cfg, defcfg, meta);
  };
  ...
  chart.prototype = import$(Object.create(Object.prototype), {
    ...
    parse: function(){
      var dim, ref$, bind;
      dim = (ref$ = this.mod).dimension || (ref$.dimension = {});
      bind = this.binding;
        ...
        for (k in ref$ = dim) {
          ...
          ret[k] = !v.multiple
            ...
            : (b.map
              ? b
              : [b]).map(fn$);
          if (/[RIT]/.exec(v.type) && !/[CN]/.exec(v.type)) {
            ret[k] = Array.isArray(ret[k])
              ? ret[k].map(fn1$)
              : _nparse(ret[k]);
          }
        }
    ...
    config: function(){
      ...
      for (i$ = 0, to$ = arguments.length; i$ < to$; ++i$) {
        i = i$;
        withDefault(o, arguments[i], this.mod.config);
      }
      withDefault(o, this.cfg, this.mod.config);
      withDefault(o, this._defcfg, this.mod.config);
      return import$(this.cfg, o);
    },
    _normalizeBinding: function(k){
      var k, res$, i$, len$, n, d, b, a, results$ = [];
      if (k == null) {
        res$ = [];
        for (k in this.mod.dimension || {}) {
          res$.push(k);
        }
        k = res$;
      }
      ...
      for (i$ = 0, len$ = k.length; i$ < len$; ++i$) {
        n = k[i$];
        d = (this.mod.dimension || {})[n] || {};
        b = this.binding[n];
        a = Array.isArray(b);
        if (!d.multiple !== !a && (d.multiple || a)) {
          results$.push(this.binding[n] = a
            ? b[0]
            : b
              ? [b]
              : []);
