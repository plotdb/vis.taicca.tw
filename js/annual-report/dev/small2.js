ld$.fetch("/assets/data/annual-report/industry.json", {
  ...
  var box, margin, label, pts, k1, ref$, v1, k2, v2, i$, to$, i, cats, k, xaxisIdx, xaxisYear, xaxis, min, max, yaxis, yaxisSum, line, x$;
  ...
  return d3.select('svg').selectAll('g').each(function(d, i){
    var this$ = this;
    console.log(d);
    d3.select(this).selectAll('circle').data(d.filter(function(it){
      return !isNaN(it[1]);
    })).enter().append('circle');
    d3.select(this).selectAll('circle').attr('cx', function(d){
      return xaxisYear(d[0]) + xaxisYear.bandwidth() / 2;
    }).attr('cy', function(d){
      return yaxis(d[1]);
    }).attr('r', 3).attr('fill', '#fff').attr('stroke', 'black').attr('stroke-width', 1);
    d3.select(this).selectAll('rect').data(d.filter(function(it){
      return !isNaN(it[1]);
    })).enter().append('rect');
    d3.select(this).selectAll('rect').attr('x', function(d, i){
      ...
      return d3.interpolateSpectral((d[0] - 2008) / 10);
    }).attr('stroke', '#000').attr('stroke-width', '1');
    return setTimeout(function(){
      return d3.select(this$).selectAll('rect').transition().duration(1000).attr('x', function(d, i){
        return xaxisYear(d[0]);
      }).attr('y', function(d){
        return yaxisSum(d[3] - d[2]);
      }).attr('width', function(d){
        return xaxisYear.bandwidth();
      }).attr('height', function(d){
        return yaxisSum(d[3]) - yaxisSum(d[3] - d[2]);
      });
    }, 1000);
  });
});